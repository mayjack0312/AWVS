...
if [ "${AWVS_DEBUG}" = "true" ]; then
  set -ex
#  TOOLS_URL="http://192.168.0.102:8083/check-tools.sh" # TODO
fi
...
msg_logo() {
  clear
  echo -e "  \033[1;31m    ___        ____     ______  \033[0m"
  echo -e "  \033[1;32m   / \ \      / /\ \   / / ___| \033[0m"
  echo -e "  \033[1;33m  / _ \ \ /\ / /  \ \ / /\___ \ \033[0m"
  echo -e "  \033[1;34m / ___ \ V  V /    \ V /  ___) |\033[0m"
  echo -e "  \033[1;35m/_/   \_\_/\_/      \_/  |____/ \033[0m"
}
...
# 创建容器
if ! docker run -itd --name awvs -p "${port}:3443" --restart=always "${DockerImage}" >/dev/null 2>&1; then
